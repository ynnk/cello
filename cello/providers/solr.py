...
class SolrIndex(Index):
    """ Access an L{Index} stored in a solr db .
        Early version !
        * get solr index
    ...
    def iter_docnums(self, incr=1000):
        start = 0
        count = len(self)
        params = {'q': "%s:*"%self.uniq_key,'fl':self.uniq_key,'start':0,'rows':incr}
        while (start < count ):
            params['start'] = start
            result = self.conn.search(**params).result
            for doc in result.docs:
...
class SolrSearch(Optionable):
    ...
    def __call__(self, query, search_field='*', qf="", fl="*,score",nb_res=10, operator="AND"):
        ...
        def _query(lucenequery, term):
            ...
            if len(term)> 0 :
                if operator == "AND":
                    if term[:1] == "-" : return lucenequery & ~qargs(term) # & not
                    else : return lucenequery & qargs(term)
                elif operator == "OR":
                    return lucenequery | qargs(term)
            else : return lucenequery
        ...
        if query:
            ...
            if search_field == "*" : 
                ...
                params.update({'defType':'dismax', 'qf':qf})
